...
import { auth } from '../base'//gives acess to the auth object which has initialized authentication
...
export default function AuthProvider({children}) {
    //Create hooks for currentUser as well as another custom hook to determine if the context has info to share
    ...
    async function login() {
        return (signInWithPopup(auth, githubAuthProvider).then(authData => {
            ...
            //This point in login logic is where we could add additional functionality such as role assignment or saving
            //the user to a local database if one is present
        }))
    }
    ...
    //The object below will hold currentUser info and login/logout functions so we can use them in the child components.
